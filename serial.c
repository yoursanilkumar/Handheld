...
SerialSend(uint32_t ui32Port, uint8_t ui8Char)
{
	...
	{
		ROM_GPIOPinWrite(GPIO_PORTH_BASE,GPIO_PIN_0,GPIO_PIN_0);
...
////		if(ui32Port == 0)
////		{
////			while(((*((volatile unsigned long *)0x4000C018))& 0x00000008) == 0x08)
////			{
...
////	}
	...
	if(GPIOPinRead(GPIO_PORTD_BASE,GPIO_PIN_2) & GPIO_PIN_2)
	{
		ROM_GPIOPinWrite(GPIO_PORTH_BASE,GPIO_PIN_0,0);
		ROM_GPIOPinWrite(GPIO_PORTD_BASE,GPIO_PIN_0,0);
	}
}


void Serial_Stringlen(uint32_t port,char*buff,uint32_t len)
{
	while(len--)
	{
		SerialSend(port,*buff);
		buff++;
	}
	
}

void Serial_String(uint32_t port,char*buff)
{
	while(*buff != '\0')
	{
